...
function provideGetPythonAgentHandlers() {
    return function getPythonAgentHandlers(pythonAgentPath) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, hasInitializeHandler, hasBlockHandler, hasTransactionHandler, pythonHandler;
            return __generator(this, function (_b) {
                _a = hasHandlers(pythonAgentPath), hasInitializeHandler = _a.hasInitializeHandler, hasBlockHandler = _a.hasBlockHandler, hasTransactionHandler = _a.hasTransactionHandler;
                if (!hasBlockHandler && !hasTransactionHandler)
                    throw new Error("no handlers found in " + pythonAgentPath);
                pythonHandler = getPythonHandler(pythonAgentPath);
                return [2, {
...
function hasHandlers(agentPath) {
    ...
    return { hasTransactionHandler: hasTransactionHandler, hasBlockHandler: hasBlockHandler, hasInitializeHandler: hasInitializeHandler };
}
function getPythonHandler(agentPath) {
    var agentModule = agentPath.replace("" + process.cwd() + path_1.default.sep, '').replace('.py', '').replace(path_1.default.sep, '.');
    ...
    var pythonWrapperPath = path_1.join(os_1.default.tmpdir(), "forta_agent_" + randomInt + ".py");
